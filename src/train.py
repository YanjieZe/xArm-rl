...
def evaluate(env, agent, video, num_episodes, L, step, test_env=False, args=None):
	...
	for i in range(num_episodes):
		...
		if args.use_wandb and i==0 and step%args.log_train_video==0 and not args.remove_addition_log:
...
def main(args):
	...
	from algorithms.per import PrioritizedReplayBuffer, EfficientPrioritizedReplayBuffer
	...
	# assert args.image_size==84, "recommended size is 84x84 for 3d algorithm."
	...
	for step in range(start_step, args.train_steps+1):
		if done:
			if step > start_step:
				...
				if step % args.log_train_video == 0 and args.observation_type!="state":
					if args.use_wandb and not args.remove_addition_log:
			...
			if step % args.eval_freq == 0:
				...
				if args.train_3d:
					...
					camera_param = info["camera_RT"] if args.use_gt_camera else None
					agent.gen_interpolate(images_rgb, camera_param, step)

    						

			# Save agent periodically

			if step!=0 and step % 500000==0 or step == args.train_steps:# step > start_step and step % args.save_freq == 0:
				torch.save(agent, os.path.join(model_dir, f'{step}.pt'))
				if args.use_wandb and args.save_model_in_wandb:
			...
			if args.use_gt_camera:
				agent.save_camera_intrinsic(info["camera_intrinsic"]) # save intrinsic, for repeated usage
			cur_camera = info["camera_RT"] if args.use_gt_camera else None
			done = False
		...
		next_camera = info["camera_RT"] if args.use_gt_camera else None
